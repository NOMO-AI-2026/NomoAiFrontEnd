import { Mail, Lock, Eye, ArrowRight, ArrowLeft } from 'lucide-react';
...
export default function LoginPage() {
  return (
    <div
      dir="rtl"
      className={`${styles.pageBackground} flex items-center justify-center min-h-screen p-6 font-sans text-[#1E1B4B]`}
    >
      {/* تم توسيع الحاوية إلى max-w-6xl لتعطي مساحة أكبر عند دفع القسم لليسار */}
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        
        {/* القسم الأيسر بالكامل (النص والصورة) */}
        {/* تمت إضافة lg:mr-24 هنا لدفع هذا القسم بالكامل لشمال الصفحة */}
        ...
        {/* كارت تسجيل الدخول (في الجانب الأيمن) */}
        <div className={`bg-white p-8 lg:p-10 ${styles.cardShadow} order-2 lg:order-1`}>
          ...
          <form className="space-y-6">
            <div className="space-y-2">
              ...
              <div
                className={`flex items-center bg-white px-4 py-3.5 ${styles.inputContainer}`}
              >
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <a
                  href="#"
                  className="text-[11px] font-extrabold text-[#581C87] hover:underline"
                >
                  نسيت كلمة المرور؟
                </a>
              </div>
            ...
            <div className="flex items-center pt-2">
              <input
                type="checkbox"
                id="keep-signed-in"
                className="w-4 h-4 text-[#581C87] border-2 border-[#581C87] rounded cursor-pointer"
              />

              <label
                htmlFor="keep-signed-in"
                className="mr-3 text-xs font-extrabold text-[#1E1B4B] cursor-pointer"
              >
                تذكرني
              </label>
            </div>
            ...
            <button
              type="button"
              className={`w-full bg-[#FACC15] text-[#581C87] font-extrabold text-[15px] py-3.5 rounded-full flex justify-center items-center gap-2 mt-2 ${styles.buttonShadow}`}
            >
              تسجيل الدخول
